 ...
 *
 * @details This file exercises environment regression test behavior for
 ...
 * - None documented; validation or dependency failures may propagate.
 */
semaforr::domain::PathDecisionPoint traversal(std::uint64_t id, double start_x,
                                              double start_y, double finish_x,
                                              double finish_y) {
  semaforr::domain::PathDecisionPoint point;
  point.selection.decision_id = id;
  point.selection.action_id = id;
  point.selection.action = {semaforr::domain::ActionType::Forward, 1U};
  point.selection.expected_start = {{start_x, start_y},
  ...
  point.execution.final_pose = {{finish_x, finish_y},
                                semaforr::domain::Angle::zero()};
  point.execution.distance_achieved_m =
      semaforr::domain::distance({start_x, start_y}, {finish_x, finish_y})
          .meters();
...
semaforr::domain::CompletedPath path(
    std::uint64_t id, semaforr::domain::PathDecisionPoint point) {
...
TEST(EnvironmentRegression, HallwayNetworkLearnsBothTravelOrientations) {
  ...
  for (std::size_t index = 0U; index < 4U; ++index) {
    ...
    horizontal.push_back(
        path(index + 1U, traversal(index + 1U, -4.0, offset, 4.0, offset)));
    vertical.push_back(
        path(index + 10U, traversal(index + 10U, offset, -4.0, offset, 4.0)));
  }
  ...
  EXPECT_TRUE(
      std::any_of(horizontal_model.hallways.begin(),
                  horizontal_model.hallways.end(), [](const auto& hallway) {
...
TEST(EnvironmentRegression, HighwayCrossingOperationalizesThroughIntersection) {
  ...
  spatial.highways.graph.vertices = {{0U, {0, 0}, {-4.0, 0.0}, true},
  ...
  const auto result = planner.plan(
      {{{-4.0, 0.0}, semaforr::domain::Angle::zero()}, {0.0, 4.0}, &spatial});
  ...
  EXPECT_TRUE(std::any_of(
      ...
      [](const auto& step) {
        ...
        return intersection != nullptr && intersection->intersection_id == 1U;
      }));
}
...
TEST(EnvironmentRegression, LargeRoomIsAnExplicitExplorationCue) {
  ...
  view.laser.angle_increment =
      semaforr::domain::Angle(3.1415926535897932 / 359.0);
  ...
  EXPECT_TRUE(std::all_of(
      candidates.begin(), candidates.end(), [](const auto& candidate) {
        return candidate.kind == semaforr::exploration::PassageKind::LargeRoom;
      }));
}
...
TEST(EnvironmentRegression,
  ...
  policy.robot_radius_m = policy.safety_clearance_m =
      policy.localization_uncertainty_m = policy.dynamic_obstacle_margin_m =
          0.0;
  const auto blocked = semaforr::planning::deriveTraversability(
      semaforr::planning::OccupancySourceMode::StaticMapWithSensors, &map,
      &sensed, policy);
...
TEST(EnvironmentRegression, NegativeCoordinateMapRoutesWithinBounds) {
  ...
  const auto result =
      planner.plan({{{-2.5, -0.5}, semaforr::domain::Angle::zero()},
                    {2.5, -0.5},
                    nullptr,
                    nullptr,
                    &map});
